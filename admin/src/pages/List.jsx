...

        {
          list.map((item, index) => (
            <div className='grid grid-cols-[1fr_3fr_1fr] md:grid-cols-[1fr_3fr_1fr_1fr_1fr_1fr_1fr] items-center gap-2 py-1 px-2 border text-sm' key={index}>
              ...
              <p>{item.category}</p>
              <p>₹{item.price}</p>
              <p>{item.rentPrice ? `₹${item.rentPrice}` : '-'}</p>
              <p>{item.discount ? `${item.discount}%` : '-'}</p>
              <div className='flex gap-2 justify-end md:justify-center'>
                <button
                  onClick={() => navigate(`/edit/${item._id}`)}
                  className='px-2 py-1 bg-blue-500 text-white rounded hover:bg-blue-600 text-xs transition-all duration-200'
                >
                  Edit
                </button>
                <span onClick={()=>removeProduct(item._id)} className='cursor-pointer text-lg text-red-500 hover:text-red-700'>X</span>
              </div>
            </div>
          ))
        }
